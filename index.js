...
const circles = new Array(25)
    .fill(undefined)
    .map(_ => new Circle(randomX(), randowY(), randomVelicity(), randomVelicity(), 30))

function Rectangle(x, y, dx, dy, width) {
    this.x = x
    this.y = y
    this.dx = dx
    this.dy = dy
    ...
    this.update = function () {
        console.log(new Date())
        if (this.x + width > window.innerWidth || this.x - width < 0) {
            this.dx = -this.dx
        }
        if (this.y + width > window.innerHeight || this.y - width < 0) {
            this.dy = -this.dy
        }
...
const recs = new Array(25).fill(undefined).map(_ => new Rectangle(randomX(), randowY(), randomVelicity(), randomVelicity(), 30))
